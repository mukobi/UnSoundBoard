...
* Currently, there is no option to play audio to a specific output device. This is a very important feature for an application like this (such that one might send its output to a virtual microphone input for use over voice chat applications), and it is planned a future release. As a workaround for now, in Windows, you can edit "App volume device preferences" in your settings which will let you choose the default output device for UnSoundBoard specifically (UnSoundBoard must be open for this to take effect).
